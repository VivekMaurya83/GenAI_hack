document.addEventListener('DOMContentLoaded', () => {
    ...
    resumeOptimizeForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (!currentResumeId) { showError("Resume ID is missing."); return; }
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/optimize`, { method: 'POST', body: formData });
            if (!response.ok) { throw await response.json(); }
            const data = await response.json();
    ...
    linkedinOptimizeForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/linkedin-optimize`, { method: 'POST', body: formData });
            if (!response.ok) { throw await response.json(); }
            const data = await response.json();
    ...
    function displayLinkedInContent(data) {
        ...
        if (data.headlines && data.headlines.length > 0) {
            html += '<h3>Headline Suggestions</h3><ul>';
            data.headlines.forEach(h => { html += `<li>${h}</li>`; });
            html += '</ul>';
        }
    ...
    function setButtonLoading(button, isLoading, loadingText) {
        ...
        button.textContent = isLoading ? loadingText : button.form.id === 'optimize-form-resume' ? 'Optimize & Download DOCX' : 'Generate LinkedIn Content';
    }
